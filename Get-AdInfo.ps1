...
try {
    ...
    $props = $($adForest.psobject.Properties | Where {$_.TypeNameOfValue -eq "System.String"}).Name
    $table = "Name,Value"
    foreach ($p in $props) {
        $v = $adForest.Item("$p")[0]
        $table += "`n`"$p`",`"$v`""
    ...
    $props = $($adDomain.psobject.Properties | Where {$_.TypeNameOfValue -eq "System.String"}).Name
    $table = "Name,Value"
    foreach ($p in $props) {
        $v = $adDomain.Item("$p")[0]
        $table += "`n`"$p`",`"$v`""
    ...
    $ous = Get-ADOrganizationalUnit -Credential $Credential -Filter * -Server $Server | Select Name,DistinguishedName

    Write-Host "getting domain controllers"
    $dclist = Get-ADDomainController -Credential $Credential -Filter * -Server $Server |
        Select Name,Domain,Site,HostName,IPv4Address,IsGlobalCatalog,IsReadOnly,OperatingSystem,OperatingSystemVersion,OperationsMasterRoles |
            % {
                [pscustomobject]@{
                    Name          = $_.Name
                    Domain        = $_.Domain 
    ...
    $groups = Get-ADGroup -Credential $Credential -Filter * -Server $Server | 
        Where-Object {$_.GroupCategory -eq 'Security'} |
            Select Name,GroupScope,ObjectGUID,DistinguishedName
    ...
    $subnets | Select Name,Site | Export-Excel -Path $xlFile -WorksheetName "Subnets" -ClearSheet -AutoSize -AutoFilter -FreezeTopRowFirstColumn
